let GIFT_S = [1, 10, 4, 12, 6, 15, 3, 9, 2, 13, 11, 7, 5, 0, 8, 14];
let GIFT_S_inv = [13, 0, 8, 6, 2, 12, 4, 11, 14, 7, 1, 10, 3, 9, 15, 5];
...
const main = () => {
  ...
  let P = [];
  let K = [];
  for (let i = 0; i < 32; i++) P[i] = (Math.random() * 16) & 0xf;
  for (let i = 0; i < 32; i++) K[i] = (Math.random() * 16) & 0xf;
...
exports.encrypt = (input, masterkey, no_of_rounds, print_details) => {
  let key = [];
  for (let i = 0; i < 32; i++) {
    key[i] = masterkey[i];
  }
  ...
  let key_bits = [128];
  let temp_key = [32];
  for (let r = 0; r < no_of_rounds; r++) {
    for (let i = 0; i < 32; i++) {
      input[i] = GIFT_S[input[i]];
    }

    if (print_details) {
      console.log("after subcel");
      for (let i = 0; i < 32; i++) {
        process.stdout.write(`${input[i]},`);
      }
    }
...
exports.decrypt = (input, masterkey, no_of_rounds, print_details) => {
  let key = [32];
  for (let i = 0; i < 32; i++) {
    key[i] = masterkey[i];
  }
  ...
  var round_key_state = new Array(no_of_rounds);
  ...
  for (let r = no_of_rounds - 1; r >= 0; r--) {
    for (let i = 0; i < 32; i++) {
      for (let j = 0; j < 4; j++) {
        bits[4 * i + j] = (input[i] >> j) & 0x1;
      }
